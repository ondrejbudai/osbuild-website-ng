...
To access protected container resources a `containers-auth.json(5)` file can be used, see [Container registry credentials](../image-builder-on-premises/container-auth.md).
...
wish to install a package from a third-party repository, please continue reading [here](../image-builder-on-premises/repository-customizations.md).
